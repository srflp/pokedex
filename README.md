...
- **2026-05-09** — Replaced Redux Toolkit + Redux Saga with TanStack Query for server state, and React Router with TanStack Router. Filter, search and page state moved from Redux into typed URL search params, so list views are now shareable and reload-safe. Type-checking now runs through the TypeScript Native Preview (`tsgo`) — `tsc --noEmit` was replaced with `tsgo --noEmit` in the build script, and a `pnpm typecheck` script was added; `moduleResolution` switched from `node10` to `bundler`. TypeScript bumped to 5.9.
