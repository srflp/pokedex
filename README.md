...
- **2026-05-09** — Bumped Prettier to v3 (default `trailingComma` is now `"all"`, so a few files were reformatted) and added a `format` GitHub Actions workflow that runs `pnpm format:check` on pull requests. Added `format` / `format:check` scripts and a `.prettierignore` covering build output and the generated `routeTree.gen.ts`.
- **2026-05-09** — Bumped React and `react-dom` to 19 (and `@types/react`/`@types/react-dom` to match), upgraded `styled-components` to v6 (drops the separate `@types/styled-components` package; custom DOM-leaking props moved to transient `$`-prefixed props), and enabled the React Compiler. Since `@vitejs/plugin-react` v6 dropped the top-level `babel` option, the compiler is wired via `@rolldown/plugin-babel` + the plugin's exported `reactCompilerPreset()`. Also bumped `@types/node` to v24 to match the project's pinned Node version.
- **2026-05-09** — Replaced Redux Toolkit + Redux Saga with TanStack Query for server state, and React Router with TanStack Router (file-based routing via `@tanstack/router-plugin/vite`; `routeTree.gen.ts` is generated by the plugin and committed alongside the source). Filter, search and page state moved from Redux into typed URL search params, so list views are now shareable and reload-safe. Type-checking now runs exclusively through the TypeScript Native Preview (`tsgo` from `@typescript/native-preview`) — the JS-based `typescript` package was removed; `tsc --noEmit` was replaced with `tsgo --noEmit` in the build script, and a `pnpm typecheck` script was added. `moduleResolution` switched from `node10` to `bundler`.
