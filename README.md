...
- **2026-05-09** — Added `tsgo --noEmit` as a required PR check (collapsed alongside `prettier --check` into a single `checks.yml` workflow with parallel jobs), and dropped the type-check step from the `build` script so Netlify's deploy preview only runs `vite build`.
