...
- **2026-05-17** — Ran `checks` and `e2e` workflows on pushes to `main` (in addition to PRs). For e2e, replaced `JakePartusch/wait-for-netlify-action` with `autotelic/action-wait-for-status-check`, which polls Netlify's GitHub commit status (`netlify/alledex/deploy-preview` for PRs, `netlify/alledex/deploy` for production) so push-to-main runs exercise the just-deployed commit instead of stale production. The preview URL is now derived from the PR number (`deploy-preview-<n>--alledex.netlify.app`).
