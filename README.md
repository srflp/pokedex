...
- **2026-05-09** — Replaced `styled-components` (now in maintenance mode and incompatible with the React Compiler) with **vanilla-extract** for zero-runtime, fully type-safe styles. Each component now has a sibling `*.css.ts` (`style()`, `recipe()` for variant-driven components, `createVar()` + `assignInlineVars()` for runtime hex colors driven by `pokemonTypeColors`), and the `createGlobalStyle` block moved to `src/styles/global.css.ts` (`globalStyle`). `@vanilla-extract/vite-plugin` is wired ahead of `react()` in `vite.config.ts` so the React Compiler downstream sees plain class strings. Removed the `styled-components` dep entirely; the gzipped JS shrank by ~11 kB (mostly the styled-components runtime out of `preload-helper`) against ~2 kB of new CSS. Migrated styled defs in `Button.tsx`, `BaseComponents.tsx` (dropped unused `Row`/`Col`; `BrightSection` now takes `as: "section" | "main"` instead of styled-components polymorphism), `Header.tsx`, `Footer.tsx` (collapsed the `styled(rawFooter)` className-passthrough wrapper), `Progress.tsx`, `Search.tsx`, `PageNumberInput.tsx`, `PageNav.tsx`, `Tile.tsx` (collapsed the `styled(TileBase)` self-reference; dropped vendor-prefixed `image-rendering` fallbacks since `pixelated` is universally supported now), `TypeFilter.tsx`, and `PokemonView.tsx`. Renamed transient `$hide`/`$selected`/`$type`/`$color` props to plain prop names. Set the previously-placeholder `allowBuilds.esbuild` to `true` in `pnpm-workspace.yaml` so vanilla-extract's esbuild postinstall runs. Annotated two pre-existing a11y issues (`<img onClick>` in `Tile.tsx`, `<p onClick>` in `PageNav.tsx`) that styled-components had hidden from oxlint's `jsx-a11y/click-events-have-key-events` rule with inline disables — they are behavior-preserving and a11y is out of scope for this migration.
- **2026-05-09** — Banned manual React memoization APIs (`memo`, `useMemo`, `useCallback`) since this repo runs the React Compiler. Added an `oxlint` `no-restricted-imports` rule keyed on the `react` package that warns with a custom message, and removed the existing call sites (a `memo()`-wrapped styled `Shadow` in `Tile.tsx`, two `useMemo`s in `ListView.tsx`, six `useCallback`s across `Tile.tsx`, `PageNumberInput.tsx`, and `PageNav.tsx`). Hoisted `handleFocus` in `PageNumberInput.tsx` to module scope to satisfy `unicorn/consistent-function-scoping`.
- **2026-05-09** — Dropped `React.FC` annotations and switched to the automatic JSX runtime: `tsconfig.json` now uses `"jsx": "react-jsx"` (Vite already used the automatic runtime — TS just hadn't been aligned), so files no longer need `import React from "react"` to bring `React` into JSX scope. Replaced remaining `React.X` references (`useRef`, `memo`, `ChangeEvent`, `RefObject`, `KeyboardEvent`, `FocusEvent`, `SyntheticEvent`) with named/`type` imports, and disabled oxlint's stale `react/react-in-jsx-scope` rule.
