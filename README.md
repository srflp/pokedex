...
- **2026-05-09** — Added a `typecheck` GitHub Actions workflow running `tsgo --noEmit` on every PR, and dropped the type-check step from the `build` script so Netlify's deploy preview only runs `vite build`.
